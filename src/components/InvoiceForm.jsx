  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (currentDataStr !== initialDataStr) {
        try {
          await createFormRecord(combinedData, 'invoice');
  ...
  const handleExcelExport = async () => {
    ...
    try {
      ...
      const fileName = selectedLanguage === 'tr' 
        ? `TUANA_FATURA_${formData['INVOICE NUMBER']}.xlsx`
        : `TUANA_INVOICE_${formData['INVOICE NUMBER']}.xlsx`;
  ...
  return (
    <div className="proforma-form-container">
      <div className="proforma-form-header">
        <h2>INVOICE</h2>
        <p>FATURA BİLGİLERİNİ DOLDURUN</p>
      </div>
